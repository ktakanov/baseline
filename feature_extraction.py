...
def create_row(df, func, min_item_id, num_of_items, dtype, fill_with_nans=False):
    result = func(df)
    index = [] if len(result.index) == 0 else (result.index - min_item_id)
    array = np.zeros(num_of_items, dtype=dtype)
    if fill_with_nans:
        array.fill(np.nan)
    array[index] = result.as_matrix()
    return np.array([array])

# getting result of computations for every feature in F1...F7 into data frames
def get_resulting_data_frame(grouped, func, dtype, fill_with_nans=False):
    group_keys = grouped.groups.keys()
    min_item_id = grouped['Item ID'].min().min()
    max_item_id = grouped['Item ID'].max().max()
    num_of_items = max_item_id - min_item_id + 1
    list_of_rows = [create_row(grouped.get_group(key), func, min_item_id, num_of_items, dtype, fill_with_nans) for key in group_keys]
    matrix = np.concatenate(list_of_rows)
    return pd.DataFrame(matrix, index=list(group_keys), columns=list(range(min_item_id, max_item_id + 1)))


def extract_f1(grouped):
    return get_resulting_data_frame(grouped, lambda x: pd.DataFrame([True], index=[x['Item ID'].iloc[0]], dtype=bool), dtype=bool)


def extract_f2(grouped):
    return get_resulting_data_frame(grouped, lambda x: pd.DataFrame([True], index=[x['Item ID'].iloc[-1]], dtype=bool), dtype=bool)


def extract_f3(grouped):
    return get_resulting_data_frame(grouped, lambda x: x['Item ID'].value_counts(), dtype=np.float16, fill_with_nans=True)
...
def extract_f4(grouped):
    ...
    return get_resulting_data_frame(grouped, time_spent, dtype=np.float16)
...
def extract_f6(grouped):
    ...
    return get_resulting_data_frame(grouped, sequent_clicks, dtype=np.int16)
...
def extract_f7(grouped):
    ...
    return get_resulting_data_frame(grouped, max_duration_between_sequent_clicks, dtype=np.float16)
...
def extract_buys(clicks_gb, buys_gb):
    clicks_session_id_list = list(clicks_gb.groups.keys())
    ...
    index = np.array(list(buys_gb.groups.keys()), dtype=np.int32) - min_session_id
